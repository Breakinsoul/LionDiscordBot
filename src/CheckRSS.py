            ...
            for entry in feed.entries:
                if entry.title not in all_threads:
                    ...
                    if file_name is not None:
                        ...
                        new_thread, thread_message = await channel.create_thread(name=entry.title, embed=news_embed, file=file)
                        await new_thread.edit(locked = True)
                    else:
                        new_thread, thread_message = await channel.create_thread(name=entry.title, embed=news_embed)
                        await new_thread.edit(locked = True)
